"""
API routes for brand voice analyzer.
"""
...
from app.agents.brand_voice_analyzer import invoke_analyzer

router = APIRouter(
    prefix="/brand-voice-analyzer",
    tags=["brand-voice-analyzer"],
...
async def compare_content_with_brand_voices(
    ...
    # Get all brand voices and check access
    brand_voices = []
    for voice_id in brand_voice_ids:
        db_voice = db.query(BrandVoice).filter(BrandVoice.id == voice_id).first()
        if not db_voice:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Brand voice with ID {voice_id} not found"
            )
        ...
        if current_user.tenant_id != db_voice.tenant_id and current_user.role != UserRole.ADMIN:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Not authorized to access brand voice with ID {voice_id}"
            )
        ...
        brand_voices.append(brand_voice_dict)
    ...
    sorted_results = sorted(
        successful_results,
        key=lambda x: x.get("analysis_result", {}).get("overall_score", 0),
        reverse=True
    )
    ...
    return {
        "success": True,
        "comparison_results": sorted_results,
