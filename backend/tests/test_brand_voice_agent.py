"""
Test script for the brand voice agent to verify functionality after refactoring.
"""
...
from app.agents.brand_voice_agent import (
...
def test_direct_tool_calls():
    ...
    create_input = BrandVoiceCreateInput(
    ...
    frame.f_back.f_locals['state'] = {"tenant_id": TEST_TENANT_ID, "user_id": TEST_USER_ID}
    
    create_result = create_brand_voice(create_input)
    ...
    generate_input = ContentGenerationInput(
        brand_voice_id=brand_voice_id,
        prompt="Write a short product description for a new smartphone",
        content_type="product"
    )
    
    generate_result = generate_content(generate_input)
    ...
    analyze_input = ContentAnalysisInput(content=sample_content)
    analyze_result = analyze_content(analyze_input)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Uncomment to test the agent workflow
        # test_agent_workflow()
