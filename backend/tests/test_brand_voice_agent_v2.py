"""
Test script for the new brand voice agent implementation (v2) to verify functionality.
"""
...
from app.agents.brand_voice_agent_v2 import (
    invoke_brand_voice_agent,
    CreateBrandVoiceTool,
    GenerateContentTool,
    AnalyzeContentTool
)
...
def test_direct_tool_calls():
    ...
    create_tool = CreateBrandVoiceTool()
    generate_tool = GenerateContentTool()
    analyze_tool = AnalyzeContentTool()
    
    # Test create_brand_voice tool
    ...
    generate_result = generate_tool.invoke({
    ...
    analyze_result = analyze_tool.invoke({
